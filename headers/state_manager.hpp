...
char grid_decision(const std::vector<vector<int>> &grid, const int &current_x,
                   const int &current_y, const int &max_w, const int &max_h,
                   const int &x, const int &y);

// find a way to a cordinate on a grit
void grid(const int &x, const int &y, const int &max_w = 0,
          const int &max_h = 0);
