...
void set_current_speed(const int &speed) { current_speed = speed; }

int get_current_angle() { return current_angle; }

void set_current_angle(const int &angle) { current_angle = angle; }

char get_current_direction() { return current_direction; }
 ...
 */
void turn_to_object(const char &movement, const int &degrees,
                    const bool &ultra_sonic_turn) {
 ...
 */
void inverse_degrees(int &degrees) { degrees *= -1; }
 ...
 */
char find_in_grid(const vector<vector<int>> &grid, const int &current_x,
                  const int &current_y, const int &max_y, const int &max_x,
                  int find) {
 ...
 * @return
 */
char find_best_way(const vector<vector<int>> &grid, const int &current_x,
                   const int &current_y, const int &max_y, const int &max_x) {
  cout << "best way" << endl;
 ...
 */
char grid_decision(vector<vector<int>> &grid, const int &current_x,
                   const int &current_y, const int &max_x, const int &max_y,
                   const int &x, const int &y, char direction) {
...
void grid(const int &x, const int &y, const int &max_x, const int &max_y) {
  ...
  while (true) {
    if (current_x < 0 || current_x > max_x || current_y < 0 ||
        current_y > max_x) {
      cout << "\nOf grid" << endl;
      stop();
      break;
    ...
    if ((color_get_reflection() > 60 && light_get_reflection() > 60) || cross) {
      ...
      if (current_x == x && current_y == y) {
        ...
        new_direction = grid_decision(grid, current_x, current_y, max_x, max_y,
                                      x, y, direction);
        for (unsigned int y = 0; y < grid.size(); y++) {
          for (unsigned int x = 0; x < grid[y].size(); x++) {
            if ((y + 1 <= max_y && grid[y + 1][x] == 2) &&
                (x + 1 <= max_x && grid[y][x + 1] == 2)) {
              grid[y][x] = 2;
            } else if ((y + 1 <= max_y && grid[y + 1][x] == 2) &&
                       x + 1 > max_x) {
              grid[y][x] = 2;
